...
function getBubbleSurface(signal: FlowSignal["signal"]): string {
  if (signal === "accumulation") {
    return "border-emerald-500/40 bg-[radial-gradient(circle_at_30%_30%,rgba(74,222,128,0.45),rgba(6,26,20,0.96)_68%)] text-emerald-50 shadow-[0_16px_40px_rgba(22,163,74,0.25)]";
  }
  if (signal === "distribution") {
    return "border-rose-500/40 bg-[radial-gradient(circle_at_30%_30%,rgba(251,113,133,0.45),rgba(35,9,14,0.96)_68%)] text-rose-50 shadow-[0_16px_40px_rgba(225,29,72,0.22)]";
  }
  return "border-slate-500/40 bg-[radial-gradient(circle_at_30%_30%,rgba(148,163,184,0.34),rgba(10,17,28,0.96)_68%)] text-slate-100 shadow-[0_16px_40px_rgba(15,23,42,0.4)]";
}
...
function TimelineCluster({ timeline }: { timeline: FlowTimelineBucket[] }) {
  ...
  return (
    <div className="rounded-2xl border border-stealth-700 bg-stealth-950/55 p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-stealth-300 sm:grid-cols-4">
...
function BubbleCluster({ rows, selectedSymbol, onSelect }: { rows: FlowSignal[]; selectedSymbol: string | null; onSelect: (symbol: string) => void }) {
  ...
  return (
    <div className="rounded-[28px] border border-stealth-700 bg-[radial-gradient(circle_at_top,rgba(46,67,96,0.5),rgba(10,15,26,0.95)_70%)] p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-[11px] uppercase tracking-[0.22em] text-stealth-500">Signal Constellation</div>
          <div className="mt-1 text-sm text-stealth-300">Bubble size tracks traded activity. Click a bubble to focus the detail card.</div>
        </div>
...
function FlowFocusCard({ row, groupScale, groupTitle }: { row: FlowSignal; groupScale: number; groupTitle: string }) {
  ...
  return (
    <article className="rounded-[28px] border border-stealth-700 bg-[linear-gradient(180deg,rgba(19,27,40,0.98),rgba(9,14,24,0.98))] p-5 shadow-[0_16px_50px_rgba(0,0,0,0.28)]">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-2xl border border-stealth-700 bg-stealth-900/60 p-3">
      ...
      <div className="mt-5 rounded-2xl border border-stealth-700 bg-stealth-950/55 p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className={`mt-1 text-lg font-semibold ${flowTone}`}>{formatCompactCurrency(row.net_flow_usd)}</div>
          </div>
      ...
      {timeline.length > 0 && <div className="mt-5"><TimelineCluster timeline={timeline} /></div>}

      <div className="mt-5 rounded-2xl border border-stealth-700 bg-stealth-950/45 p-4">
