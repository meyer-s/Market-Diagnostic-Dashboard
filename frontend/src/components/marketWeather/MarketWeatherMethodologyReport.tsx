            ...
            >
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                ...
                <MethodBlock title="Dictionary relationship scopes">
                  Directional phase places pressure on x and pressure change on y with independently auto-fit signed axes. Structure / information places the implemented Structure coordinate on x and Information / ordinal disorder on y, each fixed from 0 to 1. Scale propagation places propagation on x from 0 to 1 and cascade bias on y from −1 to +1. Every panel joins derivative and strata observations by timestamp and projects the same exact request-local Form centroids and current observation.
                </MethodBlock>
                <MethodBlock title="Scope trace, smoothing, and color">
                  A faint dashed line preserves the exact raw path. The brighter trace applies a causal exponential display average with α=0.5 (the standard span parameter is 3) and round line caps only to reduce visual aliasing; it does not feed the field, Form learning, state assignment, outcomes, or any other analysis. The diamond and centroids remain exact. Trace color and redundant width classify the display-smoothed third measure below −1, within ±1, or above +1 model-fit robust-scale units as low, typical, or elevated: structure for direction, reorganization for structure / information, and boundary activity for propagation.
                </MethodBlock>
                ...
                <MethodBlock title="Relative price and Pair scopes">
                  Relative price is indexed to 100 at the first common aligned full-precision close and is kept separate from field-coordinate differences. The beta-adjusted log-return differential starts after 20 prior aligned pairs and uses an intercept-inclusive beta estimated from at most 60 strictly prior pairs. The displayed differential does not subtract the fitted intercept and is not an OLS residual or alpha. A benchmark return standard deviation below 1e−7, a nonfinite estimate, or absolute beta above 25 makes the row unavailable; beta is rejected rather than clipped or carried, and the current contiguous beta-adjusted chain resets. It never rescales the 15 field coordinates. The family-balanced own-history-relative separation compares mean absolute context gaps with five bars earlier over the same supported coordinate intersection and reports both values and its tolerance. Pair scopes offer 12, 24, 72, or full trails and either an explicit compare-subject scale or a labeled fit-selected-trail view. Their strokes are split into chronological age bands, with opacity increasing from the oldest to the newest observation independently of screen position; screen-space simplification preserves endpoints and the strongest turns while unsupported intervals remain broken. The displayed interpretation changes with basis and subject: fit-relative coordinates are described as deviations from each instrument's frozen fit history rather than as absolute strengthening or weakening. Target, benchmark, and difference retain redundant solid/circle, dashed/diamond, and dotted/square identities in addition to hue; marker size encodes the absolute third coordinate only relative to the visible trail and missing marker evidence remains unavailable. Pointer inspection selects the nearest plotted observation in two dimensions, while keyboard arrows move through supported chronology and synchronize the inspected date across scopes. Coordinate history uses signed microbars around a centerline with magnitude in height and intensity. Chain starts or restarts and terminal direction arrows are display aids. These trajectories do not identify a winner, cycle, attractor, lead–lag mechanism, or causal transmission.
                </MethodBlock>
                <MethodBlock title="Pair identity and authority">
                  The ordered comparison hash binds target and benchmark analysis hashes plus alignment and normalization rules. Swapping the instruments reverses signed differences and changes the receipt. Request-local Form IDs and centroids are never matched across instruments. A deterministic descriptive summary translates authoritative Pair fields without adding a score or forecast. The exported frozen receipt separately binds exact shared keys, latest values, support, compatibility disclosures, and the zero-authority contract; its self-hash is an identity check, not provider certification. Pair v1 has zero scanner, outcome-learning-canary, veto, verdict, sizing, and execution authority; basket fields, connectedness, cross-sectional peer ranks, and cross-timeframe fusion remain future work.
                </MethodBlock>
                <MethodBlock title="Pair reading hierarchy">
                  Overview presents the deterministic description, relative progress and exact index base date, current beta-adjusted chain, exact field-separation classification, notable own-history-relative gaps, all-window support separately from current-coordinate support, and observed session status separately from independent certification. The user-selected comparator is labeled by reference role with suitability unevaluated. Field detail contains basis and subject controls, relationship scopes, and the 15-coordinate explorer. Audit receipt contains exact window/support/compatibility disclosures, calculation identities, authority, request-local backend timing boundaries, and export controls. This changes disclosure order, not the Pair calculation.
                </MethodBlock>
                <MethodBlock title="Live recipe versus frozen receipt">
                  Pair URLs preserve target, comparator, timeframe, requested history, recipe, basis, displayed subject, selected coordinate, active tab, scope trail and scale, and coordinate ordering. They rerun against then-current provider/cache data and do not preserve the hover date, mobile-scope selection, response values, or summary wording. Export compact receipt · JSON preserves a self-checking, unkeyed calculation checksum over shared keys and latest evidence—not full chart histories, a digital signature, proof of origin, or a provider attestation.
                </MethodBlock>
                ...
                <MethodBlock title="Raw data and this report">
                  Raw profile tables, provenance, and response-specific settings allow the visuals to be checked. Raw Data lazy-mounts only when opened, and the report mounts at most one detailed chapter at a time. For pair reports, the versioned URL also records benchmark, comparison basis and series, selected coordinate, Overview/Field/Audit tab, scope trail and scale, and coordinate ordering. It recreates a live selector recipe, not a frozen market-data snapshot; the separate JSON export preserves the current Pair receipt.
                </MethodBlock>
              </div>
              <div className="mt-3">
