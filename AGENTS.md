...
- After the final architecture-affecting state, run `.\.agents\skills\graphify-codebase\scripts\graphify.ps1 sync`. If the task creates a commit, run `sync` again after the commit so the receipt and offline constellation match the committed HEAD.
