...
| Pairwise Relative Field construction and API | backend/app/services/market_weather_comparison.py and backend/app/api/market_weather.py; exact supplement in docs/papers/market-field/relative-field-addendum.tex | Ordered, same-recipe comparison of two independently computed fields; supported coordinate-scale differences, evaluation-only proper-fit-relative differences, full-precision normalized relative price context, same-intersection family-balanced separation, exact window/support/compatibility disclosures, deterministic translation, and distinct comparison/frozen-receipt identities. The optional prior-only beta uses 20--60 prior aligned returns, is unavailable below benchmark return standard deviation `1e-7` or for nonfinite/absolute estimates above 25, and never clips or carries unavailable values; chain interruption and recovery remain explicit. Its centered slope is intercept-inclusive, but the displayed beta-adjusted return does not subtract that intercept and is not OLS alpha. This is not a basket, cross-symbol Form identity, connectedness estimate, or efficacy result. |
| Pairwise hierarchy, scopes, summary, and linkable controls | frontend/src/components/marketWeather/MarketWeatherComparisonLab.tsx, frontend/src/pages/MarketWeatherRadar.tsx, frontend/src/utils/marketWeatherPairSummary.ts, and frontend/src/utils/marketWeatherQuery.ts | Overview/Field detail/Audit receipt disclosure order, fixed subject colors, explicit scale/trail controls, synchronized inspection, deterministic server-first translation, and URL-addressable selectors. A live recipe is not frozen evidence; a compact receipt is not full chart replay. Visual relationship traces do not establish leadership, cycles, causation, or forecast skill. |
