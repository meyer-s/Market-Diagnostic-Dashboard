# Market Field future-work triage
...
| Pairwise Relative Field panel | Two independently computed same-recipe fields can be aligned into direct model-scale and fixed own-history-relative coordinate differences, full-precision relative price context, exact requested/available/returned windows, all-window bilateral support, compatibility disclosures, ordered component/comparison hashes, and a self-checking compact frozen receipt. A deterministic summary translates only authoritative fields. The three-tab hierarchy, one selected relationship scope, all-15-coordinate drilldown, 12/24/72/full trails, explicit shared/inspection scale, chronological source-time opacity, broken unsupported runs, distinct supported endpoint labels, synchronized cursor, receipt export, mobile guided flow, and linkable selectors are implemented. The Overview separates all-window from current support, names the relative-index base date, and labels each user-selected comparator role with suitability unevaluated. Redundant stroke/shape/signed-microbar encodings and visible beta-chain restarts reduce dependence on color. Daily/weekly observations align by declared session date; timezone-aware intraday timestamps normalize to UTC before exact matching, while timezone-naive rows use exact serialized timestamps without a UTC claim. Neither branch carries values. The optional prior-only beta-adjusted path requires 20--60 prior aligned returns, rejects benchmark return standard deviation below `1e-7` and nonfinite or absolute beta above 25, never clips/carries an unavailable estimate, and exposes chain resets/recovery. The canonical DXY comparison preserves Yahoo `DX-Y.NYB` as an explicit provider alias. | Pair v1 is descriptive and has zero scanner, canary, veto, verdict, sizing, or execution authority. The receipt checksum is unkeyed and compact rather than a signed or full-replay archive. Request-local Forms are not matched across instruments. Comparator appropriateness, certified sessions/currency/adjustment metadata, prospective predictive value, economic value, and reader-comprehension benefit remain unresolved. |
| Pair release and operational instrumentation | A committed browser gate retains review captures at 1440/1024/768/390, runs serious/critical WCAG A/AA axe checks, keyboard/dialog and reduced-motion checks, a deterministic reflow proxy, overflow/control checks, and response-contract assertions. A machine-readable route probe validates Pair schema, 15 coordinates, alignment/support, zero authority, and receipt identity. Request-local runtime metadata and `Server-Timing` separate cache/build/backend-response-ready stages. Bounded browser events distinguish rendering from substantial Overview visibility and record selected interactions and same-session Secret Options navigation. | Captures are not pixel-diff baselines; viewport/reflow emulation is not a real-device or literal-zoom audit; axe is not a manual screen-reader or color-vision review. Backend response-ready is not first useful summary or interactive charts. Product telemetry is not an authenticated durable exposure ledger, and a probe does not establish a stable cold/concurrent latency distribution. |
...
| Simplification and family ablations | Run the frozen prequential harness on stable pressure/family subsets, higher-derivative removals, entropy/coherence alternatives, and the full 15D vector under the same baselines and chronology. | New prospective observations are still required before any variant can be promoted; retained-cache comparisons remain development evidence. |
| Human comprehension evaluation | Freeze representative single/pair tasks and compare raw formulas, the former dense interface, and the conclusion--evidence--definition--audit hierarchy on answer accuracy, time, confidence calibration, and chosen action. | New participant evidence and a predeclared analysis plan; implementation clarity is not itself proof of comprehension. |
...
| Basket, cross-sectional, and cross-timeframe panel | Extend the pair receipt into versioned constituent baskets and cache-only panels with per-cell timestamps, weights, missingness, initialization evidence, and no fused authority. | Point-in-time constituent and weight vintages, rebalancing/corporate-action rules, session and currency conventions, plus repeated out-of-sample validation before a fused score is defensible. Pair v1 does not satisfy these requirements. |
...
- Historical proof that a person saw a particular field or rank. Server
  responses, page views, and bounded Pair product events are not equivalent to
  an authenticated durable candidate-level exposure ledger.
