...
| Pairwise Relative Field panel | Two independently computed same-recipe fields can be aligned into native and fixed proper-fit-relative coordinate differences, full-precision relative price context, shared-support receipts, ordered component/comparison hashes, and common-axis scope plots. “Native” denotes the implemented coordinate scale, not raw market units. Daily/weekly observations align by declared session date. Timezone-aware intraday timestamps normalize to UTC before exact matching; timezone-naive rows use exact serialized timestamps without being relabeled as UTC. Neither branch carries values, and nonidentity session compatibility remains unknown. The optional prior-only beta-adjusted path requires 20--60 prior aligned returns, rejects benchmark return standard deviation below `1e-7` and nonfinite or absolute beta above 25, and never clips or carries an unavailable estimate. Its centered slope is intercept-inclusive, but the displayed return does not subtract that fitted intercept and is not OLS alpha. The canonical DXY comparison preserves Yahoo `DX-Y.NYB` as an explicit provider alias. | Pair v1 is descriptive and has zero scanner, canary, veto, verdict, sizing, or execution authority. Request-local Forms are not matched across instruments. Provider/session/currency differences remain limitations, and pair instrumentation is not evidence of leadership, connectedness, prediction, or economic value. |
